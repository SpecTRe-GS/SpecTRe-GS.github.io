...
class VideoComparison {
    constructor(container) {
        ...
        this.videos = container.find('video');
        ...
        this.videos[0].style.height = "0px";  // Hide video without stopping it
        this.videos[1].style.height = "0px";  // Hide video without stopping it
    ...
    resize() {
        const videoWidth = this.videos[0].videoWidth;
        const videoHeight = this.videos[0].videoHeight;
    ...
    play() {
        ...
        // console.log('Playing video', this.videos[0], this.videos[1])
        this.isPlaying = true;
        this.videos[0].play();
        this.videos[1].play();
        this.videos[0].currentTime = this.videos[1].currentTime;
        this.videos[1].currentTime = this.videos[0].currentTime;
        this.drawLoop();
    }
    ...
    playWhenReady() {
        // console.log('play when ready', this.videos[0], this.videos[1])
        const self = this;
        if (self.videos[0].readyState >= 3 && self.videos[1].readyState >= 3) {
            self.play();
    ...
    drawLoop() {
        const self = this;
        const video0 = this.videos[0];
        const video1 = this.videos[1];
        const container = this.container;
        const context = this.context;
        requestAnimationFrame(drawFrame);

        function drawFrame() {
            const videoWidth = video0.videoWidth;
            const videoHeight = video0.videoHeight;
            const canvasWidth = container.width();
            const canvasHeight = canvasWidth * videoHeight / videoWidth;
            const position = self.position;

            context.drawImage(video0, 0, 0, videoWidth, videoHeight, 0, 0, canvasWidth, canvasHeight);
